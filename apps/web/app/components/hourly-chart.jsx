...
export function HourlyChart({ candles }) {
  const dataset = useMemo(() => {
    ...
    const yTicks = [minClose, minClose + range / 2, maxClose].map((value) => ({
  ...
  return (
    <div className="chart-wrapper">
      <svg className="chart-svg" viewBox={`0 0 ${WIDTH} ${HEIGHT}`} role="img" aria-label="Hourly price chart">
        ...
        {dataset.ticks.map((tick) => (
          <g key={tick.label} transform={`translate(${tick.x}, ${HEIGHT - PAD_Y})`}>
